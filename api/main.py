...
to any HTTP client. The Streamlit frontend is the primary consumer; other
systems (Power BI custom connectors, AlloyTower's website) can integrate
the same way.
...
ARTIFACTS_DIR = Path("model/models")
...
class PropertyInput(BaseModel):
    ...
    Fields match the cleaned dataset's column names exactly so we don't have
    to translate between schemas.
    """
    ...
    property_type: str = Field(..., examples=["Condo"],
    ...
    model_config = ConfigDict(json_schema_extra={"example": {
        ...
        "owner_occupied": True, "assessed_value": 1100000,
    }})
...
class ComparablesRequest(BaseModel):
    ...
    target_price: Optional[float] = Field(None, gt=0,
                                            description="Optional: restricts to ±20% band")
...
# CORS — Streamlit on a different port needs to be allowed
app.add_middleware(
    CORSMiddleware,
...
def load_artifacts() -> None:
    ...
    if not MODEL_A_PATH.exists() or not MODEL_B_PATH.exists():
        log.error(f"Model bundles not found in {ARTIFACTS_DIR}/. "
                  f"Did you run train_avm.py first?")
        raise RuntimeError("Model bundles missing")

    log.info("Loading AVM models...")
    predictor = AVMPredictor(MODEL_A_PATH, MODEL_B_PATH)
    log.info(f"Loaded Model A (MAPE {predictor.mape_a*100:.1f}%) "
    ...
    if SimilarPropertiesFinder is not None and CLEAN_CSV_PATH.exists():
        ...
        log.warning("Comparables service disabled: similar_properties module or "
...
def predict(prop: PropertyInput) -> dict:
    ...
    return {
        ...
        "contributions": [
            {"feature": c.feature, "display_name": c.display_name,
             "value": c.value, "contribution": c.contribution,
...
def comparables(request: ComparablesRequest) -> list[dict]:
    if finder is None:
        raise HTTPException(503, "Comparables service not available "
                                  "(similar_properties module or dataset missing)")
