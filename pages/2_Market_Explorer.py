...
st.set_page_config(
    page_title="Market Explorer | AlloyTower AVM", page_icon="📊", layout="wide"
)

page_header(
    "Market explorer", "Filter and explore the Phase 1 dataset of 1,956 properties."
)
...
with st.sidebar:
    ...
    sel_states = st.multiselect(
        "State", states, default=[], help="Leave empty to include all states."
    )

    types = sorted(df["property_type"].unique())
    sel_types = st.multiselect(
        "Property type", types, default=[], help="Leave empty to include all types."
    )

    price_min, price_max = float(df["last_sale_price"].min()), float(
        df["last_sale_price"].max()
    )
    sel_price = st.slider(
        "Price range",
...
with c2:
    st.metric(
        "Median price/sqft",
        f"${filtered['price_per_sqft'].median() if 'price_per_sqft' in filtered.columns else (filtered['last_sale_price'] / filtered['sqft']).median():.0f}",
    )
with c3:
...
chart = (
    ...
    .encode(
        x=alt.X("sqft:Q", title="Living area (sqft)", scale=alt.Scale(zero=False)),
        y=alt.Y(
            "last_sale_price:Q", title="Sale price ($)", axis=alt.Axis(format="$,.0f")
        ),
        color=alt.Color(
            "property_type:N",
            title="Property type",
